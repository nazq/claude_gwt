  ...
  describe('CLI basic operations', () => {
    it('should show help', async () => {
      try {
        ...
        expect(code).toBe(0);
        // Check for current help output format (deprecation notice or main help)
        expect(stdout).toMatch(
          /Git Worktree Manager with Claude Code Orchestration|WARNING.*deprecated|Guided setup experience/,
        );
        expect(stdout).toMatch(/--repo|--help/);
      } catch (error) {
